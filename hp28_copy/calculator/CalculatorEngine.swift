...
class CalculatorEngine
{
    ...
    var mode = Mode.trig {
        didSet
        {
            ...
            case Mode.plot:
                self.modeText = "Plot"
                funcMode = false
            case Mode.inverse_units:
                self.modeText = "Inv. Units"
            }
        }
    }
    var degreeText = "deg"
    ...
    func funcButtonPressed(funcButton: Func)
    {
        ...
        case Func.button_i:
            print("button i")
            if self.mode == Mode.units
            {
                self.mode = Mode.inverse_units
            }
            else
            {
                if self.mode == Mode.inverse_units
                {
    ...
    func modeButtons(row r:Int, col c:Int) -> String
    {
        ...
        case Mode.units:
            buttons.append(contentsOf: ["°C→K", "°C→°F", "→", "→", "→"])
            buttons.append(contentsOf: ["kg/h→kg/s", "g/min→kg/s", "knop→km/h", "→", "→"])
            buttons.append(contentsOf: ["→", "→", "→", "→", "→"])
            
        case Mode.inverse_units:
            buttons.append(contentsOf: ["K→°C", "°F→°C", "→", "→", "→"])
            buttons.append(contentsOf: ["kg/s→kg/h", "kg/s→g/min", "km/h→knop", "→", "→"])
            buttons.append(contentsOf: ["→", "→", "→", "→", "→"])
