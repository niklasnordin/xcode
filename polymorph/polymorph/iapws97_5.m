...
static NSString *name = @"iapws97_5";

static int nCoeffs = 6;
static int n0Coeffs = 6;

@implementation iapws97_5
...
-(iapws97_5 *)initWithArray:(NSArray *)array
{
    self = [super init];
...
-(long double)d2gdt2ForP:(long double)p andT:(long double)T
{
    ...
    {
        a = powl(pi, _ii[i]);
        b = _ji[i]*(_ji[i]-1.0)*powl(tau, _ji[i]-2.0);
        sum += _ni[i]*a*b;
    }
